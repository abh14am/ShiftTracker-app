...
export default function NewShift({ userAuth }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [m, s] = await Promise.all([getTeamMembers(), getShiftTypes()]);
        setMembers(m);
        setShiftTypes(s);
        if (m.length > 0) {
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: '800px', margin: '0 auto' }}>
      <header style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '2rem', marginBottom: '0.25rem' }}>New Handover</h1>
        <p style={{ color: 'var(--text-secondary)' }}>Record the details of the active shift.</p>
      </header>
